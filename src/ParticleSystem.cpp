...
void ParticleSystem::setVelocity(ofVec3f temp) {
    for (int i = 0; i < particles.size(); i++) {
...
ImpulseRadialForce::ImpulseRadialForce(float magnitude, float yMin, float yMax) {
    this->magnitude = magnitude;
    minY = yMin;
    maxY = yMax;
    applyOnce = true;
...
void ImpulseRadialForce::updateForce(Particle * particle) {
	...
	//
	ofVec3f dir = ofVec3f(ofRandom(-1, 1), ofRandom(minY, maxY), ofRandom(-1, 1));
